...
//		this.add(test);
	...
	private String checkstaffinfo(String idtemp, String phonetemp) {
		String result = "";
	...
	private String checkorginfo(String orgid, String orgname) {
		String result = "";
	...
	private void addstaff() {
		...
		if (checkempty(temp)) {
			String result = checkstaffinfo(idtemp, phonetemp);
			if (result.isEmpty()) {
	...
	private void addorg() {
		...
		if (checkempty(temp)) {
			String result = checkorginfo(orgid, orgname);
	...
	private void addbankaccount() {
		...
		if (checkempty(temp)) {
			if (checkaccount(incomestr, expensestr)) {
				...
				String result = checkaccountinfo(accountname, total + "");
	...
	private void changestaff(int row) {
		...
		if (complete) {
			...
			String result = checkstaffinfo(id, phone);
			if (!result.isEmpty()) {
				ok = false;
			}
	...
	private void changebankaccount(int row) {
		...
		if (complete) {
			...
			if (result.isEmpty()) {
				...
				BankAccountVO vo = new BankAccountVO(accountname, income, expense,
						total);
